...
# # Contents
#
# - [ ] fix info regarding partitions and quotas
# - [ ] RAM vs disk /dev/shm VS /tmp
...
Provide an overview of various tips and tricks for using the CalcUA HPC and hopefully make you more productive.
...
# Connecting your editor - remote SSH {.smaller}
...
## Acknowledging the `vim`/`emacs` users out there (and a little tip regarding dotfiles) {.smaller}
...
These tools save their settings in dotfiles (small configuration files that live in your home directory). Things like your `.bashrc` or `.bash_aliases` file also fall in this category. If you'd like to streamline your environment between your own machine and remote servers like CalcUA, you can store and sync your dotfiles via git or a dedicated tool like [yadm](https://yadm.io/) or [chezmoi](https://www.chezmoi.io/comparison-table/).
...
# Persistent sessions using `tmux`/`screen`/`Zellij`

<!-- TODO info on other tools that do the same thing - tiling window managers - built-in panes in terminal's like kitty/wezterm -->
